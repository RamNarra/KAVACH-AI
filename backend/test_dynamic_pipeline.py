...
SESSION_HEADERS = {"X-Kavach-Session": "sess_test_dynamic_pipeline_1234567890"}
...
def main() -> None:
    ...
    if not os.path.exists(local_apk_path):
        raise SystemExit(f"Error: Local APK not found at {local_apk_path}")
    ...
    try:
        with httpx.Client(timeout=240.0) as client:
            ...
            if response.status_code == 200:
                ...
                raise SystemExit(f"Static Analysis FAILED: {response.text}")
    except Exception as e:
        raise SystemExit(f"API post failed: {e}") from e

    if not doc_id:
        raise SystemExit("Failed to acquire Document ID. Exiting.")
    ...
    if not static_completed:
        raise SystemExit("Static analysis did not complete in time or failed. Exiting.")
    ...
    try:
        ...
        raise SystemExit(f"API dynamic post failed: {e}") from e
